...
def simplify_palavras( ctxinfo, pos ) :
    """
    ...
    if pos == "N" or pos == "V" or pos == "PCT" or pos == "NUM" :
        ...
        try :
            ...
            ctxinfo.warn("Part of speech {pos!r} not converted", pos=newpos)
    return newpos                                  

################################################################################

def simplify_ptb( ctxinfo, pos ) :
    """
        Receives as input a complex POS tag in the Penn Treebank format (used by 
        treetagger) and return a simplified version of the same tag.
...
args = util.read_options( "xF:pg", longopts, treat_options, -1, usage_string )
