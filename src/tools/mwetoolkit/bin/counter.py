...
from libs.base.webFreq import MAX_CACHE_DAYS, DEFAULT_LANG
...
from libs.filetype.indexlib import Index, ATTRIBUTE_SEPARATOR, index_escape

################################################################################
# GLOBALS    

usage_string = """\
Usage: {progname} [-w | -y | -u <id> | -T <dir> | -i <index-corpus>] OPTIONS <candidates>
...
You must choose exactly one of -w or -i
...
--corpus-from <corpus-filetype>
    Only works if the `--index` switch has been given as well.
...
--cache-filename <filename>
    Name of cache file storing web frequencies. (default: ./google_cache.dat)

--max-cache-days <days>
    Maximum number of days that a web result sould stay in the cache file. A
    ...
    daily limit of 5000 searches. (default: -1)
...
--lower <bound>
    Lower bound to retrieve frequencies. Any candidate or word having a
    frequency of less than <bound> will have no frequency assigned.
    (default: -1, meaning no limit)

--upper <bound>
    Upper bound to retrieve frequencies. Any candidate or word having a
    frequency greater than <bound> will have no frequency assigned.
    (default: -1, meaning no limit)

-o OR --old
    Use the old (slower) Python indexer, even when the C indexer is available.
    Not advisable for large corpora. Remember that, to use the C indexer by
    default, you must run "make" in the toolkit's root folder.
...
# TODO: Add --attributes <attrs>
    ...
    # XXX 2015-10-07 shouldn't we have only --attr="lemma+pos+@foobar" meaning
    # "use all these attributes"?  And then we deprecate "-s" and "-g".
        ...
        if ( self.entity_counter >= low_limit or low_limit < 0 ) and \
            ...
            if count_vars:
                for var in candidate.vars:
                    append_counters(ctxinfo, var)
            else:
                append_counters(ctxinfo, candidate)
...
def append_counters(ctxinfo, ngram):
    """
    ...
    for w in ngram:
        ...
        c_pos.append(w.pos)
        freq_value = get_freq_function(ctxinfo, [w.surface],
                                       [w.lemma],
                                       [w.pos])
    ...
    if count_bigrams:
        i = 0
        while i < len(ngram) - 1:
            freq_value = get_freq_function(ctxinfo, c_surfaces[i:i + 2],
...
def get_freq_index(ctxinfo, surfaces, lemmas, pos):
    ...
    for i in range(len(surfaces)):
        word = build_entry(index_escape(surfaces[i]),
                           ...
                           index_escape(pos[i]))
        wordid = suffix_array.symbols.escaped_symbol_to_number.get(word, None)
        if wordid:
...
def get_freq_web(ctxinfo, surfaces, lemmas, pos):
    """
        Gets the frequency (number of occurrences) of a token (word) in the
...
def get_freq_web1t(ctxinfo, surfaces, lemmas, pos):
    """
    ...
    if length > 5:
        ctxinfo.warn("Cannot count the frequency of an n-gram, n>5!")
        return 0
    ...
    util.verbose("WEB1T: Opening %s, looking for %s" % (filename, search_term))
    ...
    if line:
        ...
        freq = 0
    util.verbose("freq =" + str(freq))
    return freq


################################################################################

def open_index(ctxinfo, prefix):
    """
...
def treat_options(opts, arg, n_arg, usage_string):
    ...
    for o, a in ctxinfo.iter(opts):
        if o in ( "-i", "--index" ):
            open_index(ctxinfo, a)
            get_freq_function = get_freq_index
            mode.append("index")
        elif o in ( "-y", "--yahoo" ):
            ctxinfo.error("THIS OPTION IS DEPRECATED AS YAHOO SHUT DOWN THEIR "
                  "FREE SEARCH API")
            #web_freq = YahooFreq()          
            #freq_name = "yahoo"
            #ignorepos_flag = True 
            ...
            ctxinfo.error("THIS OPTION IS DEPRECATED AS GOOGLE SHUT DOWN THEIR "
            ...
            if o == "--lower" :
                if up_limit == -1 or up_limit >= limit:
                    ...
                    ctxinfo.error("Argument of --lower >= argument of --upper")
            else:
                if low_limit == -1 or low_limit <= limit:
                    up_limit = limit
                else:
                    ctxinfo.error("Argument of --upper <= argument of --lower")
        elif o in ("-a", "--vars" ):
            count_vars = True
        elif o in ("--cache-filename") :
            cache_filename = a
        elif o in ("--max-cache-days" ):
            max_cache_days = ctxinfo.parse_signed_int(a)
        elif o in ("-l", "--lang" ):
            language = a
            ...
            Index.use_c_indexer(False, ctxinfo)
        elif o == "--corpus-from":
    ...
    if len(mode) != 1:
        ctxinfo.error("Exactly one option -w, -T or -i, must be provided")

    if mode == ["index"]:
        if surface_flag and ignorepos_flag:
            build_entry = lambda surface, lemma, pos: surface
            suffix_array = index.load("surface", ctxinfo)
        elif surface_flag:
            build_entry = lambda surface, lemma, pos: \
                    surface + ATTRIBUTE_SEPARATOR + pos
            suffix_array = index.load("surface+pos", ctxinfo)
        elif ignorepos_flag:
            build_entry = lambda surface, lemma, pos: lemma
            suffix_array = index.load("lemma", ctxinfo)
        else:
            build_entry = lambda surface, lemma, pos: \
                    lemma + ATTRIBUTE_SEPARATOR + pos
            suffix_array = index.load("lemma+pos", ctxinfo)

    elif mode == ["google"] :
        web_freq = GoogleFreq(cache_filename=cache_filename,
                              max_cache_days=max_cache_days)
        freq_name = "google"
        the_corpus_size = web_freq.corpus_size()
        get_freq_function = get_freq_web
         # Web search, entries are single surface or lemma forms
        if surface_flag:
            build_entry = lambda surface, lemma, pos: surface
        else:
            build_entry = lambda surface, lemma, pos: lemma
    ...
    #    ctxinfo.warn("-x option is recommended for web queries, not textual indices")
...
longopts = ["candidates-from=", "corpus-from=", "to=",
            ...
            "univ=", "web1t=", "max-cache-days=", "cache-filename="]
args = util.read_options("ywi:gsoal:Jbu:T:", longopts,
        treat_options, -1, usage_string)

try:
    util.verbose("Counting ngrams in candidates file")
