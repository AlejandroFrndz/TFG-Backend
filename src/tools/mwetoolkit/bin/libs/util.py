...
class InputObjContextInfo(ContextInfo):
    ...
    * @param mwetkparser: Optional[AbstractParser]
    ...
    def __str__(self):
        return '<InputObjContextInfo linenum="{}" colnum="{}">'.format(
            self.linenum.range_str() or "?", self.colnum.range_str() or "?")
    ...
    def _ranges(self):
        ...
        return self.linenum.range_prefix() + self.colnum.range_prefix()
...
class NumberRange(collections.namedtuple('NumberRange', 'beg end')):
    ...
    def range_prefix(self):
        r"""Return a human-readable string range (starts at 1) ending in ":"."""
    ...
    def range_str(self):
        ...
        return self.range_prefix().rstrip(':')
