...
class TextualPatternParser(common.AbstractTxtParser):
    ...
    def _parse_line(self, line, ctxinfo):
        ...
        self.line = line
        self.ctxinfo = ctxinfo.with_column_range(0, len(self.line))
        try:
    ...
    def glob_first_child(self, cur_ctxinfo, opener_stack):
        r"""Globs the next child assuming the given stack of opener characters.
        @type opener_stack: list[str]
        
        Return a pair (child_ctxinfo, postchild_ctxinfo), where:
        * child_ctxinfo: a ctxinfo referring to the globbed child.
        * postchild_ctxinfo: a ctxinfo referring to the point right after the globbed child.
        """
        ...
        while i < cur_ctxinfo.colnum.end:
            ...
            if not opener_stack:
                ...
                return child_ctxinfo, cur_ctxinfo  # Found the end of the globbed piece
    ...
    def parse_globbed_in_paren(self, cur_ctxinfo,
        ...
        while cur_ctxinfo.colnum.beg < cur_ctxinfo.colnum.end:
            ...
            if not opener:
                cur_ctxinfo = cur_ctxinfo.with_column_range(cur_ctxinfo.colnum.beg, None)
            ...
            if opener.group("bracket"):
                child_ctxinfo, postchild_ctxinfo = self.glob_first_child(cur_ctxinfo, ["["])
                cur_ctxinfo, w_attribs = self.read_attribs(postchild_ctxinfo)
                w = self.parse_in_word(child_ctxinfo)
                w = self.added_attribs(w, postchild_ctxinfo, w_attribs)
                children.append(w)

            # Handle: (blablaba...)
            elif opener.group("paren"):
                child_ctxinfo, postchild_ctxinfo = self.glob_first_child(cur_ctxinfo, ["("])
                cur_ctxinfo, p_attribs = self.read_attribs(postchild_ctxinfo)
                child_ctxinfo, p = self.parse_globbed_in_paren(child_ctxinfo,
                        opener.group("paren"), postchild_ctxinfo, p_attribs)
                children.append(p)
            ...
            # Handle a literal, such as `walk` or `NPP`
                ...
                cur_ctxinfo, lit_attribs = self.read_attribs(cur_ctxinfo)
                literal = opener.group('literal')
                ...
                if literal.isupper():
                    ...
                    value = patternlib.LiteralProp(child_ctxinfo, literal)
                    w.add_prop("lemma", value, False)
                w = self.added_attribs(w, cur_ctxinfo, lit_attribs)
                children.append(w)
        ...
        if paren_prefix == "either(":
            ...
            return cur_attrib_ctxinfo, self.added_attribs(p, cur_attrib_ctxinfo, cur_attribs)
        else:
    ...
    def read_attribs(self, cur_ctxinfo):
        ...
        Return (postattrib_ctxinfo, attribs), where
        * postattrib_ctxinfo points to the char after the braces;
        * attribs is a dictionary of attributes.
        """
        if not self.line.startswith("{", cur_ctxinfo.colnum.beg):
            return cur_ctxinfo, {}  # No list of attributes

        cur_ctxinfo = cur_ctxinfo.with_column_range(
            cur_ctxinfo.colnum.beg+1, cur_ctxinfo.colnum.end)
        attrib_ctxinfo, postattrib_ctxinfo = self.glob_first_child(cur_ctxinfo, ["{"])
        attribs = {}
        ...
        while attrib_ctxinfo.colnum.beg < attrib_ctxinfo.colnum.end:
            ...
            if not pair:
                self.warn_syntax(cur_ctxinfo, "Bad key=value pair")
                break  # ignore rest of key=value pairs as we can't parse the current one
            attribs[pair.group("key")] = pair.group("val")
        return postattrib_ctxinfo, attribs
    ...
    def parse_in_word(self, cur_ctxinfo):
        ...
        while cur_ctxinfo.colnum.beg < cur_ctxinfo.colnum.end:
            new_ctxinfo, pair = self.match_and_advance(cur_ctxinfo, self.RE_BRACKET_ELEM)
            if not pair: self.warn_syntax(cur_ctxinfo, "Bad prop=value pair")
            delimited, flags = False, ""

            xval = pair.group("val")
            xval_ctxinfo = cur_ctxinfo.with_column_range(
                    cur_ctxinfo.colnum.beg - len(xval), cur_ctxinfo.colnum.end)
