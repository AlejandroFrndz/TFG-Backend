...
usage_string = """\
Usage: {progname} OPTIONS <cache1.dat> <cache2.dat> <cache_out.dat>
...
longopts = []
arg = util.read_options( "", longopts, util.treat_options_simplest, 3, usage_string )

util.verbose( "Opening files and checking consistency" )
...
cache_out = {}
util.verbose( "Combining cache files..." )
combine_caches( cache1, cache2, cache_out )
util.verbose( "Writing new cache file..." )
cPickle.dump( cache_out, cache_out_desc )
util.verbose( "{c} had {n} entries".format(c=arg[ 0 ], n=len(cache1)) )
util.verbose( "{c} had {n} entries".format(c=arg[ 1 ], n=len(cache2)) )
util.verbose( "Result has {n} entries".format(n=len(cache_out)) )
