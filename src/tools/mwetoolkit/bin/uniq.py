...
class UniqerHandler(filetype.ChainedInputHandler):
    ...
    def _fallback_entity(self, entity, ctxinfo):
        ...
        internal_key = str(entity.to_string()).encode('utf-8')

        if ignore_pos :
            entity.foreach_del_prop("pos")

        if surface_instead_lemmas :
            entity.foreach_del_prop("lemma")
        else :
            entity.foreach_del_prop("surface")
            
        if internal_key not in entity_buffer:
            entity_buffer[internal_key] = (entity, ctxinfo)
        else:
            old_entity, ctxinfo = entity_buffer[internal_key]
    ...
    def finish(self, ctxinfo):
        ...
        """
        global entity_buffer
        util.verbose( "Output the unified ngrams..." )
        for uniq_counter, (entity, ctxinfo) in enumerate(entity_buffer.values()):
            ...
            if isinstance( entity, Candidate ) :
                ...
                entity.freqs.clear()
                for ( name, value ) in list(freq_sum.items()) :
...
args = util.read_options( "gst", longopts, treat_options, -1, usage_string )
