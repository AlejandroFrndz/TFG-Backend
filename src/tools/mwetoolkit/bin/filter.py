...
class FilterHandler(filetype.ChainedInputHandler):
    ...
    def _fallback_entity(self, entity, ctxinfo):
        ...
        if lenentity < minlength or lenentity > maxlength :
            print_it = False
            util.verbose("Filtered out: %d tokens" % lenentity)

        # Filter out sentences with too few/too many MWE candidates
        if entity.DISPATCH == "handle_sentence":
        ...
        if print_it :   
            self.chain.handle(ngram_to_print, ctxinfo)
...
def interpret_length( ctxinfo, l, maxormin ):
    """
    ...
    """
    result = ctxinfo.parse_uint(l)
    util.verbose("%s length: %d" % (maxormin, result))
    return result
...
def treat_options( opts, arg, n_arg, usage_string ) :
    ...
    for o, a in ctxinfo.iter(opts):
        if o in ( "-t", "--threshold" ) : 
            ...
            if threshold :
                ...
                ctxinfo.error( "The format of the -t argument must be <source>:"
            ...
            if equals :
                ...
                ctxinfo.error( "The format of the -e argument must be <name>:"
                       "<value>\n<name> must be a valid feat name and "
                       "<value> must be a non-empty string")

        elif o in ("-p", "--patterns") :
            util.verbose( "Reading patterns file" )
            global patterns
            patterns = filetype.parse_entities([a])
        elif o in ("-r", "--reverse") :
            reverse = True
            util.verbose("Option REVERSE active")

        elif o in ("-i", "--minlength") :
            minlength = interpret_length( ctxinfo, a, "minimum" )
        elif o in ("-a", "--maxlength") :
            maxlength = interpret_length( ctxinfo, a, "maximum" )
        elif o == "--min-mweoccurs":
            min_mweoccurs = interpret_length( ctxinfo, a, "minimum")
        elif o == "--max-mweoccurs":
            max_mweoccurs = interpret_length( ctxinfo, a, "maximum")
        elif o == "--from":
    ...
    if minlength > maxlength:
        ctxinfo.warn("minlength should be <= maxlength")
    if min_mweoccurs > max_mweoccurs:
        ctxinfo.warn("min-mweoccurs should be <= max-mweoccurs")
...
args = util.read_options( "t:e:p:ra:i:", longopts, treat_options, -1, usage_string )
