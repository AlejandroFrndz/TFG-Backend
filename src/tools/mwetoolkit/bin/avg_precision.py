...
from libs.base.feature import UNKNOWN_FEAT_VALUE
...
usage_string = """\
Usage: {progname} [OPTIONS] -f <feats> <candidates>
...
class StatsCollectorHandler(filetype.InputHandler):
    def handle_meta(self, meta, ctxinfo):
        ...
        if not tp_classes_ok :
            ctxinfo.error("You must define a boolean TP class")
        ...
        if not feat_list_ok :
            for feat_name in feat_list :
                if feat_name not in all_feats :
                    ctxinfo.error("%(feat)s is not a valid feature\n" + \
                          "Please chose features from the list below\n" + \
                          "%(list)s" % {"feat": feat_name,
                                        "list": "\n".join(
                                            ["* " + x for x in all_feats])})
            feat_list_ok = True
...
def calculate_map( values ):
    ...
    for ( index, ( value, tpclass ) ) in enumerate( values ) :
        if tpclass :
            ...
            #util.verbose "Precision at %(c)d : %(p)f" % { "c" : tpcounter, 
            #                                         "p" : precision }
...
def print_stats() :
    ...
    # its ID
    for tpclass in list(feat_to_order.keys()) :
        precisions = []
        print("----------------------------------------------------------------")
...
def treat_options( opts, arg, n_arg, usage_string ) :
    ...
    if not feat_list :
        ctxinfo.error("You MUST provide at least one feature with -f")

################################################################################
# MAIN SCRIPT

longopts = [ "from=", "feat=", "asc", "desc", "precs" ]
args = util.read_options( "f:adp", longopts, treat_options, 1, usage_string )
