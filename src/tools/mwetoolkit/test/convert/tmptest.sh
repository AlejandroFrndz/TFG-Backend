...
t_args_DESCR="Test conversion among different filetypes"
t_args_parse "$@"
...
/home/silvioricardoc/work/dev/mwetoolkit3-concept/bin/transform.py -v --from=Moses --to=XML /tmp/AMOSES >/home/silvioricardoc/work/dev/mwetoolkit3-concept/test/convert/output/corpus.moses.xml
...
test_bidir() {
    ...
    if test -f "$t_REFDIR/$filename"; then
        ...
        t_compare "$input1" "$output" "Comparing \"$filename\" vs original"
    fi
}
...
# test_inputOnly <filetype> <input-file>
#   Converts <input-file> to XML and compare to reference.
test_inputOnly() {
    local filetype="$1"; shift
    local filename="$1"; shift
    local input1="$t_LOCAL_INPUT/$filename"
    local output="$t_OUTDIR/${filename}.xml"
...
###########################################################
# Testing input-only formats

t_testname "Check RASP format"
test_inputOnly RASP "corpus.rasp3"
test_inputOnly RASP "corpus.rasp2"

t_testname "Check PALAVRAS format"
test_inputOnly PALAVRAS "corpus.palavras"
